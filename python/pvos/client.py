"""PeakVisionOS local Unix Socket SDK.
...
class PVOSClientError(RuntimeError):
    """A local PeakVisionOS Unix Socket request failed."""


AgentOSClientError = PVOSClientError
...
def _call(key, line, timeout=DEFAULT_TIMEOUT, raw=False, body=None):
    ...
    try:
        try:
            ...
            raise PVOSClientError(f"local PeakVisionOS request failed: {exc}") from exc
    finally:
        s.close()
    if raw:
        return data
...
class PeakVisionOS:
    """PeakVisionOS 客户端:一行 API 调感知/认知/记忆/语义文件/运行时/上下文。
...
# Compatibility alias for the AgentOS SDK 1.5 alpha import surface.
AgentOS = PeakVisionOS
