"""HTTP client for the PeakVisionOS control plane and Remote Gateway.
...
class GatewayClient:
    ...
    def __init__(self, endpoint: Optional[str] = None, token: Optional[str] = None,
        ...
        endpoint = endpoint or os.environ.get("PVOS_ENDPOINT") or os.environ.get("AGENTOS_ENDPOINT", "")
        if not endpoint:
            raise ValueError("endpoint is required (or set PVOS_ENDPOINT)")
        endpoint = endpoint.rstrip("/")
        if not endpoint.startswith(("http://", "https://")):
            raise ValueError("endpoint must use http:// or https://")
        self.endpoint = endpoint
        self.token = token if token is not None else (
            os.environ.get("PVOS_TOKEN") or os.environ.get("AGENTOS_TOKEN", "")
        )
...
class AsyncGatewayClient:
    ...
    async def _run(self, method, *args, **kwargs):
        ...
        return await loop.run_in_executor(
            None, lambda: getattr(self._client, method)(*args, **kwargs)
        )
    ...
    async def agents(self):
        return await self._run("agents")

    async def workspaces(self):
        return await self._run("workspaces")

    async def create_workspace(self, name):
        return await self._run("create_workspace", name)

    async def tasks(self, workspace_id=None):
        return await self._run("tasks", workspace_id)

    async def create_task(self, workspace_id, title, description="", agent=""):
        return await self._run("create_task", workspace_id, title, description, agent)

    async def runs(self, workspace_id=None):
        return await self._run("runs", workspace_id)

    async def create_run(self, agent, task_id="", workspace_id="",
                         manifest_digest="", idempotency_key=None):
        return await self._run("create_run", agent, task_id, workspace_id,
                               manifest_digest, idempotency_key)

    async def run(self, run_id):
        return await self._run("run", run_id)

    async def stop_run(self, run_id):
        return await self._run("stop_run", run_id)

    async def logs(self, agent):
        return await self._run("logs", agent)
