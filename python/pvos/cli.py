"""PeakVisionOS CLI —— Agent 开发、测试、打包和验收入口。
...
aos = pvos.PeakVisionOS(caller="{NAME}")   # caller 用于 inferd 算力记账
...
def cmd_acceptance(args):
    ...
    try:
        result = run_acceptance(args.endpoint, args.token, args.agent, args.timeout)
...
def cmd_uninstall(args):
    ...
    try:
        removed = uninstall_package(args.name, args.root)
    except Exception as exc:
...
def cmd_deploy(args):
    ...
    try:
        ...
        return 1
    print(json.dumps({"commands": commands, "dry_run": args.dry_run}, ensure_ascii=False, indent=2))
    return 0
...
def main(argv=None):
    ap = argparse.ArgumentParser(
        prog="pvos",
        description="PeakVisionOS SDK 命令行:生成、测试、打包、部署和验收 Agent",
    )
    ...
    p = sub.add_parser("mock-server", help="启动无需真实节点的本地控制面模拟器")
    p.add_argument("--host", default="127.0.0.1")
    p.add_argument("--port", type=int, default=17680)
    p.add_argument("--token", default=os.environ.get("PVOS_TOKEN", ""))
    p.set_defaults(func=cmd_mock_server)
    p = sub.add_parser("acceptance", help="运行控制面端到端验收")
    p.add_argument("--endpoint", default=os.environ.get("PVOS_ENDPOINT", "http://127.0.0.1:17680/api/v1"))
    p.add_argument("--token", default=os.environ.get("PVOS_TOKEN", ""))
    p.add_argument("--agent", default="mock-agent")
    ...
    p = sub.add_parser("deploy", help="通过 SSH 上传并安装 Agent 包")
    p.add_argument("package")
    p.add_argument("host")
    p.add_argument("--root", default="/etc/agent-os/agents")
...
def legacy_main(argv=None):
    """Compatibility entry point for the former ``agentos`` executable."""
    return main(argv)
